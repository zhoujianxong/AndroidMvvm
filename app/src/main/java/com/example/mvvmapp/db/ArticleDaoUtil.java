...
public class ArticleDaoUtil {
    ...
    public boolean insertArticleBean(ArticleBean articleBean){
        boolean flag = false;
        flag = mManager.getDaoSession().getArticleBeanDao().insert(articleBean) == -1 ? false : true;
        ALog.i(TAG, "insert ArticleBean :" + flag + "-->" + articleBean.toString());
        return flag;
    }

    public boolean insertArticleBean(TestBean articleBean){
        boolean flag = false;
        flag = mManager.getDaoSession().getTestBeanDao().insert(articleBean) == -1 ? false : true;
        ALog.w(TAG, "insert ArticleBean :" + flag + "-->" + articleBean.toString());
        return flag;
    }
    public List<TestBean> queryAllTestBean(){
        return mManager.getDaoSession().loadAll(TestBean.class);
    }
